...
register struct job *jp;
{
	...
	register int rc;
	/* POSIX: Use regular waitpid without WNOWAIT. 
	   We update job status after reaping. */
	while ((rc = waitpid(pid, &stat, WUNTRACED)) != pid) {
		if (rc == -1 && errno == ECHILD) {
			/* Child already reaped or doesn't exist */
			return;
		}
		if (rc == -1 && errno == EINTR) continue;
		/* Other errors, break to avoid infinite loop */
		break;
	}
...
printjob(struct job *jp, int propts)
{
	...
	if (propts & PR_STAT) {
		...
		sp = 28;
		if (jp->j_flag & J_SIGNALED) {
			if (jp->j_xval < (ushort)NSIG) {
				const char *__gtxt();
				const char *sig = __gtxt("uxlibc", jp->j_xval + 4,
					sys_siglist[jp->j_xval]);
				sp -= strlen(sig);
				prs_buff(sig);
			} else {
