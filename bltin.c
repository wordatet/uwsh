...
struct trenod	*t;
{
	...
	case SYSREAD:
		{
			...
			rwait = 1;
			exitval = readvar(&argv[1+read_rflag]);
			opterr = savopterr;
			optind = savoptind;
			rwait = 0;
			break;
	...
	case SYSGETOPT: {
        ...
        case SYSPRIV:
		prs_buff("priv: not supported on this POSIX system\n");
		exitval = 1;
		break;

	case SYSMLD:
		prs_buff("mldmode: not supported on this POSIX system\n");
		exitval = 1;
		break;
