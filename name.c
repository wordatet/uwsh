...
#ident	"@(#)sh:common/cmd/sh/name.c	1.14.12.6"
#ident  "$Header$"
/*
 * UNIX shell
 */
...
{
	int j = ltos(i);
	/* Note: assnum is used for positional params which don't have a namnod.
	   However, for variables it might be called.
	   Let's see where it's used. */
	// replace(p, &numbuf[j], 0); 
    // Wait, replacing this might be tricky if p is not from a namnod.
    // Let's check usages of assnum.
    free(*p);
    *p = make(&numbuf[j], 0);
}
	...
	register unsigned char	*nam;
{
	...
	nscan->namid = make(nam, 0); /* IDs are leaked for now to avoid complexity */
	nscan->namval = 0;
	...
	register struct namnod *n;
{
	...
	if (flg & N_ENVCHG)
	{
		...
		if (flg & N_EXPORT)
		{
			if (n->namflg & N_MALLOC) free(n->namenv);
			n->namenv = make(n->namval, n);
		}
		else
		{
			if (n->namflg & N_MALLOC) free(n->namval);
			n->namval = make(n->namenv, n);
		}
	}
...
setup_env()
{
	register unsigned char **e = sh_environ;
...
/* setup_tfm removed for POSIX compliance */
