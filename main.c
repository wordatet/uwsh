...
#ident	"@(#)sh:common/cmd/sh/main.c	1.14.33.1"
#ident  "$Header$"
/***************************************************************************
 * Command: sh
...
main(int c, char *v[], char *e[])
{
	...
	/*
	 * Get the shell timeout value from the sh_environment.
	 */
	...
	/*
	 * SVR4 specific setup_tfm() removed for POSIX compliance.
	 */
	/*
	 * 'rsflag' is zero if SHELL variable is
	 *  set in sh_environment and 
	 ...
	 */
	replace(&cmdadr, dolv[0], 0);	/* cmdadr is $0 */
	...
	if ((beenhere++) == FALSE)	/* ? profile */
	{
		...
		else
		{
			...
			if ( strncmp((char *)cmdadr, DEVFD, strlen(DEVFD)) == 0)
