...
#ident	"@(#)sh:common/cmd/sh/hashserv.c	1.10.16.5"
#ident  "$Header$"
 ...
 */
int
chk_access(name, mode, testflg)
register unsigned char	*name;
mode_t mode; 
int testflg;
{	
	struct	statvfs vfstatb;
	struct	stat	statb;

	if (mode & X_OK)		/* only needed for X_OK case */
		mode |= X_OK;

	if(access((char *)name, 0 | mode) == 0)
	{
		return(0);
	}
	else if (mode & X_OK) 
	{
		/*
		 * If access over network and the target system
		 * is a pre 4.1 machine, X_OK will result EINVAL
		 * from 4.0, EACCES from 3.x, because X_OK is 
		 * new in 4.1.
		 *
		 * To be compatible with pre 4.1, will reset the
		 * mode to its original value, do a stat and an
		 * an access as it was done in 4.0.
		 */
		...
		if (errno == EACCES)
		{
			if ((testflg==1) && (stat((char *)name, &statb) == 0))
			{
			...
			if (testflg==0 || dir==0)
			{
				(void) statvfs((char *)name, &vfstatb);
				if ( strcmp(vfstatb.f_type == 0 ? "" : "x", "unknown") != 0)
					return(3);	/* not pre 4.1 source */
			}
		}
		if ( errno == EACCES || errno == EINVAL )
		{
			/* a pre 4.1 source  or  test -x dir */
			mode &= ~X_OK;

			if ((testflg==1) || (stat((char *)name, &statb) == 0)) 
			{
				/* if testflg, don't stat again */
				if(access((char *)name, 0|mode) == 0) 
				{
					/*
					 * privileged user can execute file as long as
					 * the file has execute permission for someone
					 */
