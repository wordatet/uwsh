...
#ident	"@(#)sh:common/cmd/sh/xec.c	1.28.24.1"
#ident "$Header$"
/*
 *
 * UNIX shell
...
int	*pf1, *pf2;
{
	...
	if (t && execbrk == 0) {
		...
		case TFORK:	/*FALLTHROUGH*/
		{
			...
			if (type == TFORK)
				execute(forkptr(t)->forktre, xflags | XEC_EXECED, errorflg, pf1, pf2);
		...
		case TPAR:
			clearjobs();
			execute(parptr(t)->partre, xflags|XEC_NOSTOP, errorflg, pf1, pf2);
			done(0);
			/*NOTREACHED*/
		case TFIL:
			{
		...
		case TBRA:
			execute(parptr(t)->partre, xflags|XEC_NOSTOP, errorflg, 0, 0);
			break;

		case TLST:
			execute(lstptr(t)->lstlef, xflags&XEC_NOSTOP, eflag, 0, 0);
			execute(lstptr(t)->lstrit, xflags, eflag, 0, 0);
			break;
		...
		case TFOR:
			{
				...
				{
					assign(n, *args++);
					execute(forptr(t)->fortre, XEC_NOSTOP, errorflg, 0, 0);
					if (breakcnt < 0)
						execbrk = (++breakcnt != 0);
				}
				if (breakcnt > 0)
		...
		case TUN:
			{
				...
				loopcnt++;
				while (execbrk == 0 && (execute(whptr(t)->whtre, XEC_NOSTOP, 0, 0, 0) == 0) == (type == TWH))
				{
					i = execute(whptr(t)->dotre, XEC_NOSTOP, errorflg, 0, 0);
		...
		case TIF:
			if (execute(ifptr(t)->iftre, XEC_NOSTOP, 0, 0, 0) == 0)
				execute(ifptr(t)->thtre, xflags|XEC_NOSTOP, errorflg, 0, 0);
			else if (ifptr(t)->eltre)
				execute(ifptr(t)->eltre, xflags|XEC_NOSTOP, errorflg, 0, 0);
			else
		...
		case TSW:
			{
				...
				while (regp)
				{
					...
					while (rex)
					{
						...
						{
							execute(regp->regcom, XEC_NOSTOP, errorflg, 0, 0);
							regp = 0;
							break;
						}
						else
